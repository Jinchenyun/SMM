...
from data.Dataset import Caltech, INRIA, BCI_IV_IIa
...
for epoch in outer_pbar:
    ...
    svm = SVC(**best_param)
    svm.fit(X_train_val_clean, y_train_val_clean)
    y_pred = svm.predict(X_test_clean)
    ...
    svm = SVC(**best_param)
    svm.fit(X_train_val_gn, y_train_val_gn)
    y_pred = svm.predict(X_test_gn)
    ...
    svm = SVC(**best_param)
    svm.fit(X_train_val_spn, y_train_val_spn)
    y_pred = svm.predict(X_test_spn)
