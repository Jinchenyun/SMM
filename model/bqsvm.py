...
class BPSVM(BaseEstimator, ClassifierMixin):
    def __init__(self, C=1.0, lambda_val=1.0, tau=1.0, sigma=1.0,
                 kernel='linear', fit_intercept=True, eps=1e-5,
                 eps_cccp=1e-5, max_steps=2000, cccp_steps=100):
        """
        Bounded Quadratic SVM (BQSVM) implementation
    ...
    def _update_deltak_bq(self, f, lambda_val, tau):
        """
        Compute delta_k for BQSVM
        ...
        pos_idx = f >= 0
        if np.any(pos_idx):
            delta_k[pos_idx] = lambda_val * (1 - 1 / (1 + lambda_val * f[pos_idx]) ** 2)

        # For negative f
        neg_idx = f < 0
        if np.any(neg_idx):
            delta_k[neg_idx] = -tau * lambda_val * (1 - 1 / (1 - tau * lambda_val * f[neg_idx]) ** 2)

        return delta_k

    def fit(self, X, y):
        """
        Train BQSVM model using CCCP algorithm
        ...
        for i in range(self.cccp_steps):
            lb = -self.C * delta_k - self.lambda_val * self.tau * self.C
            ub = -self.C * delta_k + self.lambda_val * self.C
            ...
            f = 1 - H @ u0
            delta_k = self._update_deltak_bq(f, self.lambda_val, self.tau)
